...
class ProgramParameter {
	...
	/*value */
	protected $varying;
	/*varying on/varying off */
	protected $dimension;
	protected $by;
	/* val or ref */
	protected $isArray;
	/* treat as an array of similarly defined data. true or false */
	protected $labelSetLen;
	/* use on an integer field to set length there based on labelLen (see below) */
	protected $labelLen;
	/* use this on a data structure to get the size/length */
	protected $labelDoUntil = '';
	/* use on a data structure array along with 'dim' to to set # of records to
	 * return based on labelEndDo (see below) */
	protected $labelEndDo = '';
	/* use this on an integer "count" field to control the number of records to
	 * return in n array data structure (see labelDoUntil above) */
	...
	// if data field is not named, the toolkit creates a name of the pattern var0,
	// var1, var2...
	static protected $_fallbackNameSequence = 0;
	// start with zero to give unnamed elements a unique name

	// TODO do setlen for other program param types, too

	// added byval, isarray, labellen, labelsetlen
	function __construct($type, $io, $comment = '', $varName = '', $value, $varying = 'off', $dimension = 0, $by = 'ref', $isArray = false, $labelSetLen = null, $labelLen = null, $ccsidBefore = '', $ccsidAfter = '', $useHex = false) {

		// some properties are different if value is an array (implement via a data
		// structure).
		...
		// handles array with original info
	...
	// set a parameter's properties via an key=>value array structure. Choose any
	// properties to set.
	...
	protected function getFallbackVarName() {
		...
		// This works better than in the original new toolkit, where the var(n) only was
		// applied if 'var was not set as a variable at all.
	...
	protected function handleParamValue($type, $io, $comment, $varName, $value, $varying, $dimension, $by, $isArray, $labelSetLen, $labelLen, $ccsidBefore, $ccsidAfter, $useHex) {

		// added....
		// if $value is an array, but not yet a data structure, make a data structure of
		// the array elements.
		/// same for $dim
		if (is_array($value) && ($type != 'ds')) {
			...
			if ($count) {
				...
				foreach ($value as $key => $singleValue) {
					// use $key as a sequential (probably) unique-ifier, though not strictly
					// necessary
					$ds[] = new self($type, $io, "{$comment}_$key", "{$varName}_$key", $singleValue, $varying, $dimension, $by, $isArray, $labelSetLen, $labelLen, $ccsidBefore, $ccsidAfter, $useHex);
				}
	...
	// from a program call,
	// set the label for "enddo". Links up with the Dou label given to the dim'med
	...
	// "CCSID before" means how to convert to CCSID on the way in to XMLSERVICE, when
	// needed
	...
	// "CCSID after" means how to convert to CCSID on the way out from XMLSERVICE,
	// when needed
	...
	static function UpdateParameterValues(&$arrParams, array $arrValues) {
		...
		foreach ($arrValues as $varName => $newData) {
			...
			// for each value, loop through all params at this level to see if the names
			// match.
			foreach ($arrParams as $single) {
				...
				if (is_object($single) && $single->isDS()) {
					...
					if ($paramName === $varName) {
						...
						//$single->setParamValue(self::handleParamValue($newData)); // if data is an
						// array; not done right
	...
	/* bin2str is used by the 5250 Bridge. It converts a hex string to character
	 * string
	 * while cleaning up unexpected characters.
	 * Original comment: "can not be public. Return XML does not return a type of
	 * values."
	 **/
		...
		if ($arrParams instanceof ProgramParameter) {
			...
			if ($arrParams->isDS()) {
				...
				if ($arrParams->isReturn()) {
					...
					$params = array('ds' => array(
							'fields' => self::ParametersToArray($arr),
							'ds_descr' => $arrParams->getParamProperties()
						));

				} else// non-return DS. Reduce DS to simple structure('ds'=>valuearray),
					// eliminating any attributes the DS had, such as its name.
					$params[] = array('ds' => self::ParametersToArray($arr));

			} else {// single non-DS param
				$params[] = $arrParams->getParamProperties();
			}
		} else {// array of ProgramParameter objects
			foreach ($arrParams as $single) {
				if ($single->isDS()) {
					// element in array is DS
					$arr = $single->getParamValue();
					// array of values
					// Reduce DS to simple structure ('ds'=>valuearray), eliminating any attributes
					// the DS had, such as its name.
					$params[] = array('ds' => self::ParametersToArray($arr));
...
class DataStructure extends ProgramParameter {
	...
	// v1.4.0 added $comment as arg 5, in place of the obsolete $isReturnParam
	// argument. Data structure return values didn't work properly before 1.4.0
	// anyway.
...
//function __construct( $type,  $io, $comment='', $varName = '', $value, $varying
// = 'off', $dimension = 0, $by = 'ref', $isArray = false, $labelSetLen = null,
// $labelLen = null,
...
class CharParam extends ProgramParameter {
	...
	function __construct($io, $size, $comment, $varName = '', $value, $varying = 'off', $dimension = 0, $by = '', $isArray = false, $ccsidBefore = '', $ccsidAfter = '', $useHex = false) {
		$type = sprintf("%dA", $size);
		parent::__construct($type, $io, $comment, $varName, $value, $varying, $dimension, $by, $isArray, null, null, $ccsidBefore, $ccsidAfter, $useHex);
		return $this;
		// fluent interface
	}
...
// "hole" means, don't return the data where the hole is defined. A way to ignore
