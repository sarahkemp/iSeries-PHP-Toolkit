...
class DateTimeApi {
	...
	public function dtsToYymmdd($dtsDateTime) {
		...
		$paramXml = "<parm io='in' comment='1. Input format'>
          ...
          </parm>\n" . ToolkitService::getErrorDataStructXml(5);
		// param number 5
...
class ListFromApi {
	...
	protected $_receiverDs;
	// innards of a data structure <data>..</data><data>...</data> that we'll wrap in
	// a receiver variable. At this time it must be an XML string.
	...
	public function __construct($requestHandle, $totalRecords, $receiverDs, $lengthOfReceiverVariable, ToolkitService $ToolkitSrvObj = null) {
		...
		$this->_nextRecordToRequest = 1;
		// will request record #1 when someone asks
		$this->_receiverDs = $receiverDs;
		// will request record #1 when someone asks
	...
	// Embed the call to the QGYGTLE API in a do-loop that loops until a record is
	// returned.
  ...
  <parm io='in' comment='6. Starting record' >
  ...
  </parm>\n" . ToolkitService::getErrorDataStructXml(7);
		// param number 7
	...
	public function close() {
		...
		$paramXml = "<parm io='in' comment='1. request handle'>
                      <data var='requestHandle' comment='Request handle: binary/hex' type='4b'>$requestHandle</data>
                    </parm>\n" . ToolkitService::getErrorDataStructXml(2);
		// param number 2
...
class UserSpace {
	...
	// TODO in cw.php, user space api should do an error action with CPF code if
	// needed
	...
	public function CreateUserSpace($UserSpaceName = NULL, $USLib = NULL, $InitSize = 1024, $publicAuthority = '*ALL', $InitValue = ' ', $extendedAttribute = 'PF', $textDescription = 'ZS XML Service User Space') {
		...
		// single binary hex value X'00, most efficient initialization, according to
		// documentation of QUSCRTUS
	...
	public function RetrieveUserSpaceAttr() {
		...
		//$params[] = array('ds'=>$ds);
		$params[] = ToolkitService::AddDataStruct($ds, 'receiver');
		// note that ds names are discarded
		...
		// If data structure integrity turned off, ds'es are discarded when reading
		// output. The subfields become independent.
		// So 'receiver' may not exist. But this changes with data structure integrity,
		// so allow for receiver var.
	...
	public function RetrieveUserSpaceSize() {
		...
		return (isset($ret['Space Size']) ? $ret['Space Size'] : -1);
	...
	// CW version. $param must be an array of ProgramParameter objects or a single
	// ProgramParameter object.
	public function WriteUserSpaceCw($startPos = 1, ProgramParameter $param) {
		/*	if (!is_object($param) && !is_array($param)) {
		 throw new Exception('Parameter passed to WriteUserSpaceCw must be an array or
		ProgramParameter object.');
		 }
		 */
	...
	public function ReadUserSpace($frompos = 1, $readlen = 0, $receiveStructure = null) {
		...
		if ($receiveStructure) {
			...
			// wrap simple ds around receive structure so we can assign a varname to retrieve
			// later.
	...
	/*	private function DefineUserSpaceParameters( $InitSize, $Auth, $InitChar )
	 {
	 $params[] = ToolkitService::AddParameterChar('in',20, "USER SPACE NAME",
	'userspacename', $this->getUSFullName() );
	 $params[] = ToolkitService::AddParameterChar('in',10, "USER
	TYPE",'userspacetype', "PF        ");
	 $params[] = ToolkitService::AddParameterInt32('in',   "US SIZE",
	'userspacesize', $InitSize);
	 $params[] = ToolkitService::AddParameterChar('in',1,  "INITIALIZATION VALUE",
	'initval', $InitChar);
	 $params[] = ToolkitService::AddParameterChar('in',10, "AUTHORITY", 'authority',
	$Auth);
	 $params[] = ToolkitService::AddParameterChar('in',50, "COMMENTS", 'comment', "ZS
	XML Service");
	 $params[] = ToolkitService::AddParameterChar('in',10, "Replace US",
	'replaceuserspace', "*NO       ");
	 $params[] = ToolkitService::AddErrorDataStruct();
	...
	protected function generate_name() {
		$localtime = localtime();
		$this->USName = sprintf("ZS%d%d%d%d", $localtime[0], /*sec*/
		$localtime[1], /*min*/
		$localtime[2], /*our*/
		$localtime[3] /*day*/
		);
...
class DataQueue {
	...
	public function CreateDataQ($DataQName, $DataQLib, $MaxLength = 128, $Sequence = '*FIFO', $KeyLength = 0, $Authority = '*LIBCRTAUT', $QSizeMaxNumEntries = 32999, $QSizeInitNumEntries = 16) {
		$this->DataQueueName = $DataQName;
		$this->DataQueueLib = $DataQLib;

		if (strcmp(strtoupper($Sequence), '*KEYED') == 0 || strcmp(strtoupper($Sequence), '*FIFO') == 0 || strcmp(strtoupper($Sequence), '*LIFO') == 0)
			$DataQType = $Sequence;
		else
		...
		else if (strcmp($QSizeMaxNumEntries, '*MAX16MB') == 0 || strcmp($QSizeMaxNumEntries, '*MAX2GB') == 0)
		...
		$cmd = sprintf("QSYS/CRTDTAQ DTAQ(%s/%s) MAXLEN(%s) SEQ(%s) %s  AUT(%s)", $this->DataQueueLib, $this->DataQueueName, $MaxLen, $DataQType, $AdditionalSetting, $Authority);
	...
	public function DeleteDQ($DataQName = '', $DataQLib = '') {
		...
		$cmd = sprintf("QSYS/DLTDTAQ DTAQ(%s/%s)", ($DataQLib != '' ? $DataQLib : $this->DataQueueLib), ($DataQName != NULL ? $DataQName : $this->DataQueueName));
	...
	public function receieveDataQueue($WaitTime, $KeyOrder = '', $KeyLength = 0, $KeyData = '', $WithRemoveMsg = 'N') {
		...
		// TODO do not hard-code data size. Use system of labels as allowed by
		// XMLSERVICE.
		$DataLen = 300;
		$Data = ' ';
		...
		// Wait time: < 0 waits forever. 0 process immed. > 0 is number of seconds to
		// wait.
		$params[] = $this->ToolkitService->AddParameterPackDec('in', 5, 0, 'waittime', 'waittime', $WaitTime);

		if (!$KeyLength) {
			// 0, make order, length and data also zero or blank, so thatthey'll be ignored
			// by API. Must send them, though.

			// if an unkeyed queue, API still expects to receive key info,
			// but it must be blank and zero.
			$KeyOrder = '';
			// e.g. EQ, other operators, or blank
			$KeyLength = 0;
			$KeyData = '';
		...
		// Sender info may contain packed data, so don't receive it till we can put it in
		// a data structure.
		...
		$params[] = $this->ToolkitService->AddErrorDataStructZeroBytes();
...
class SpooledFiles {
	...
	/*multi user. QTemp impossible to use(?)Opened in amother job*/
	private $TmpUserSpace;
	/*LISTSPLF function filles this object by the spool file list information*/
	private $ErrMessage;
	...
	public function GetSPLList($UserName = "*CURRENT") {
		...
		$this->ToolkitSrvObj->PgmCall(ZSTOOLKITPGM, ZSTOOLKITLIB, $params, $retval, array('func' => 'LISTSPLF'));
	...
	public function GetSPLF($SplfName, $SplfNbr, $JobNmbr, $JobName, $JobUser, $TMPFName = '') {
		...
		$this->ToolkitSrvObj->ClCommandWithCpf($crtf);
		// was ClCommand

		// clear the temp file
		$clrpfm = "CLRPFM $this->TmpLib/$this->TMPFName";
		$this->ToolkitSrvObj->ClCommandWithCpf($clrpfm);
		// these all were CLCommand but we need CPFs.

		// copy spooled file to temp file
		$cmd = sprintf("CPYSPLF FILE(%s) TOFILE($this->TmpLib/$this->TMPFName) JOB(%s/%s/%s) SPLNBR(%s)", $SplfName, trim($JobNmbr), trim($JobUser), trim($JobName), $SplfNbr);
	...
	public function createJobListArray($JobListString) {
		...
		if (is_array($JobListString)) {
			...
			foreach ($JobListString as $element) {
				...
				$JobList[$i]['JOBSUBS'] = (isset($el[4])) ? $el[4] : '';
				// avoid undefined offset
				$JobList[$i]['ACTJOBSTATUS'] = (isset($el[5])) ? $el[5] : '';
				// avoid undefined offset
				$i++;
			}
		}
...
class DataArea {
	 ...
	 * use CL command */
	public function createDataArea($DataAreaName = '', $DataAreaLib = "*CURLIB", //
	// *CURLIB is correct, the default with CRTDTAARA.
	$size = 2000) {
		...
		$cmd = sprintf("QSYS/CRTDTAARA DTAARA(%s/%s) TYPE(*CHAR) LEN($dataAreaLen)", ($DataAreaLib != '' ? $DataAreaLib : $this->DataAreaLib), ($DataAreaName != '' ? $DataAreaName : $this->DataAreaName));
	...
	public function setDataAreaName($dataAreaName, $dataAreaLib = "*LIBL") {
		...
		if (in_array($dataAreaName, array(
			'*LDA',
			'*GDA',
			'*PDA'
		))) {
	...
	public function readDataArea($fromPosition = 1, $dataLen = '*ALL') {
		...
		$maxValueSize = 2000;
		// largest allowed data area size
		 ...
		 *0 	0 	BINARY(4) 	Bytes available (The length of all data available to return.
		 * All available data is returned if enough space is provided.)
		...
		$receiverVar[] = $toolkit->AddParameterChar('out', $maxValueSize, 'Value returned', 'value', '');
		// set length to $maxValueSize to be safe

		$receiverLength = $maxValueSize + 36;
		// 4 4-byte integers + 2 10-byte character strings = 36
		...
		// Starting position: The first byte of the data area to be retrieved. A value of
		// 1 will identify the first character in the data area. The maximum value
		// allowed for the starting position is 2000. A value of -1 will return all the
		// characters in the data area.
		...
		$dsIntegrity = $toolkit->getOption('dataStructureIntegrity');
		// save original value
		$toolkit->setOptions(array('dataStructureIntegrity' => true));

		$retPgmArr = $toolkit->PgmCall('QWCRDTAA', '', $toolkitParams);

		$toolkit->setOptions(array('dataStructureIntegrity' => $dsIntegrity));
		...
		/*
		 ...
		 // Send the command; get output array of key/value pairs. Example:
		// CURUSER=>FRED, ...
	...
	public function writeDataArea($value, $fromPosition = 0, $dataLen = 0) {
		...
		// TODO use API instead. Handle numeric and character data., *CHAR and *DEC as
		// well.
		$cmd = sprintf("CHGDTAARA DTAARA(%s $substring) VALUE($value)", $this->getQualifiedDataAreaName());
	...
	public function deleteDataArea($DataAreaName = '', $DataAreaLib = '') {
		$cmd = sprintf("QSYS/DLTDTAARA DTAARA(%s/%s)", ($DataAreaLib != '' ? $DataAreaLib : $this->DataAreaLib), ($DataAreaName != NULL ? $DataAreaName : $this->DataAreaName));
