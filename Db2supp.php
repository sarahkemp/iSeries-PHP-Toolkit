...
class db2supp {
	...
	// TODO define common transport class/interface extended/implemented by all
	// transports
	...
	public function connect($database, $user, $password, $options = null) {
		 ...
		 * ....
		 Throw new Exception( "Fail execute ($sql)
		".db2_stmt_errormsg(),db2_stmt_error());
		 ... and retrieve via try/catch + Exception methods.
		 */

		// check for blank password with non-blank user. If so, throw the same error that
		// wrong password would generate.
	...
	public function execXMLStoredProcedure($conn, $sql, $bindArray) {
		...
		// But added some error handling in ToolkitService.php, ExecuteProgram, looking
		// at error code.
		...
		// stored procedure takes four parameters. Each 'name' will be bound to a real
		// PHP variable
		$params = array(
			array(
				'position' => 1,
