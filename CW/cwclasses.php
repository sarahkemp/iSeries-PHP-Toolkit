...
// toolkit path should be defined in PHP.INI. Default:
...
class ToolkitServiceCw extends ToolkitService {
	...
	public function __destruct() {
		...
		// CW only: if connection is in a separate job and nonpersistent, end job
		// (mimicking behavior of old toolkit)
	 ...
	 * TODO this may not work because CPFs are done at a class level (data areas
	 * etc.)
	 */
	 ...
	 * param array $outputDesc   Format of output params 'CODE'=>'CODEvar' where the
	 * value becomes a PHP var name
	...
	public function setOutputVarsToExport(array $outputDesc, array $outputValues) {
		...
		// for each piece of output, export it according to var name given in
		// $outputDesc.
	...
	// returns true on success, false on failure (failure probably means lack of
	// authority).
...
class I5Error {
	 ...
	 *
	 * @param int    $errNum    Error number (according to old toolkit). Zero/false
	 * if no error
...
class DataDescription {
	...
	// save structs in here
	protected $_countRefNames = array();
	// names of countRef fields (fields containing counts to return from programs)

	// TODO create methods to handle then make protected
	public $_miscAttributes = array();
	// user-defined

	// array of simple types, old to new toolkit, with sprintf-style percent
	// formatting.
	protected $_typeMap = array(
		...
		I5_TYPE_SHORT => "5i0",
		I5_TYPE_ZONED => "%ss%s",
		// TODO not sure if byte really maps to binary. No one knows what BYTE really
		// does
		I5_TYPE_BYTE => "%sb",
	...
	/**
	 * Constructor takes an object name (program, data queue, user space) and
	 * array-based data description
	 * and does some conversions.
	 * @param string    $objName        name of program, data queue, etc.
	 * lib/pgm(svcfunc) or the like.
	 * @param array    $dataDescription array of parameter definitions
	 * @param I5Error  $errorObj        during validation we may set properties of
	 * this object.
	...
	public function setIsReceiverOnly($isReceiverOnly = false) {
		// turn this on when want to use default input variables because we're only
		// RECEIVING data from a program call.
	...
	public function setIsSingleLevelSimpleValue($isSingleLevelSimpleValue = false) {
		// turn this on when want to accept a parameter that's a single-level description
	 ...
	 * @return boolean   True if validates successfully, false if any invalid
	 * elements.
	 */
	...
	public function splitPcmlProgramPath($path) {
		...
		// remove the /qsys.lib that may be in front but only if it's simply qualifying
		// another library. qsys may be the actual program library, too.

		// trim not only spaces, but slashes, from beginning and end.
		// also make uppercase for consistency. (OK to use uppercase--lib and program
		// here, not function)
	 ...
	 * @return string|array|null  value found in input array for array key. null if
	 * failed
	 */
	protected function findValueInArray($searchKey, $valueArray) {
		...
		foreach ($valueArray as $key => $value) {
			...
			// a match was found!
			if ($key == $searchKey) {
				//                $valToLog = (is_array($value)) ? print_r($value, true) :
				// $value;
				//		     	$connection->logThis("findValueInArray: searchKey: $searchKey. value
				// found: $valToLog");
				return $value;
			}
		}//(foreach values)
		...
		//	    $connection->logThis("findValueInArray: searchKey: $searchKey. no value
		// found");
		return null;
	}//(findValueInArray)

	/*
	 *   Process a single parameter definition, converting from old toolkit to new
	 * toolkit style.
	 *
	 ...
	 * Name - name of the field
	 ...
	 Length
	 for CHAR, BYTE - integer describing length. Length can be number or name of the
	variable holding the length in the data
	 structure.
	 ...
	 DSParm (optional) - array of the parameter of the Data structure. Each parameter
	is defined by a data definition in the same
	...
	protected function oldToNewDescriptionItem($oldDataDescription = array(), $inputValues = null) {
		...
		// $inputValues can be a branch of the overall input tree, if this function was
		// called recursively.
		...
		$connection = $this->getConnection();
		//		$connection->logThis("desc in: " . print_r($old, true) . " value array in: "
		// . print_r($inputValues, true));
		...
		if (isset($old['count']) && $old['count']) {
			...
			// This param refers to another parameter that contains a count value (or will
			// after program execution).
			// countref takes value from a param value
			// Find the input array entry that countref references, and assign its value to
			// $dim.
			...
			// save this countref name for later use (we will later set a label on the
			// countref field itself)
			$this->addCountRefName($nameContainingCount);

			// get value of actual field referred to by countref. This will be starting count
			// for dim='count', for initializing occur arrays in called program.
			// findInputValueByName() searches recursively for the desired key.
			$countRefValue = $this->findInputValueByName($nameContainingCount, $this->getInputValues());
			// getInputValues() goes to original input array to search all params for the
			// countref field

			if (!$countRefValue) {
				// value not found for countRef. Use default of 999 for backwards compatibility
				// with older CW versions.
				// (If the called program sets the field referred to by countRef ('dou/enddo'
				// value)to a number higher than 999, the limit will still be 999.)
				$countRefValue = 999;
			}//(if (!$countRefValue))

			// if key found, value must be numeric (numeric string or integer) because it'll
			// be our count.
		...
		$inOutStuff = $io & I5_INOUT;
		// two bits. could be IN, OUT, or INOUT
		$byVal = $io & I5_BYVAL;

		$newInout = (isset($this->_inoutMap[$inOutStuff])) ? $this->_inoutMap[$inOutStuff] : 'in';
		$newBy = (isset($this->_inoutMap[$byVal])) ? $this->_inoutMap[$byVal] : '';
		// default is blank
		...
		if ($dsName) {
			...
			/* $useDefaultValues may be true in two situations:
			 ...
			 2. We are doing a normal program call but the associated input/output values are
			not present.
			 */
			if ($useDefaultValues) {
				/*
				...
				foreach ($dsParm as $subParm) {
					...
					$subParm = array_change_key_case($subParm, CASE_LOWER);
					// for consistency
					...
					$dsData[$subname] = '';
					// default value. TODO Could do 0 or something for numeric
				} //(foreach)
			 ...
			 * If we are using default values then we need to wrap it in an "array" data
			 * structure
			 * because later it will come back to us, as output, with all elements generated
			 * within that structure.
			 *
			 * If we are not using default values, let's assume that data for the array has
			 * been provided to us
			 * fully formed.
			 *
			 */
			if ($dim > 1) {
				...
				// if we use default values, treat as a single DS but employ the "dim" attribute
				// to tell XMLSERVICE to provide output of dim/array of DS.
				// if we use real values, expect an array of arrays (the fully formed array of
				// data)
				...
				// If we are expanding to a fully data-populated array then we don't need to tell
				// XMLSERVICE
				...
				if ($expandToArray) {
					...
					$dimTagValue = $dim;
					// Put an outer array around our single element so it can survive the "foreach"
					// below.
				...
				/*	$connection->logThis("array of DS found. dsName: $dsName. dsData: " .
				 * print_r($dsData, true) . ' dsParm: ' . print_r($dsParm, true));
				 */

				// Step 2: Associate description with data for each element of the DS
				//                     In this case, we have real data so we loop through each
				// array element.
				//                     If default values, works the same but with only one
				// element.
				...
				foreach ($dsData as $numericIndex => $dsDataSingle) {
					...
					//	        		$connection->logThis("recursively handle a single DS from an
					// array");
					// pass in full description of DS, including DS name etc., but with count = 1.
					$singleDsDesc = $old;
					unset($singleDsDesc['count'], $singleDsDesc['countref']);
					// single now
					// On output we will use numeric index (0, 1, 2, 3...) as "name" of array
					// elements
					//
					...
					// dim tag tells XMLSERVICE to expand this element to a certain maximum size on
					// output
					// (after the RPG/COBOL completes) to hold the output array.
					if ($dimTagValue) {
						$newStyleItem->setParamDimension($dimTagValue);
						// dim gets set at inner level
				...
				// Very important. The output parser will see this DS labeled [array='true'] and
				// know that we have an array of DSes inside.
				$new = new DataStructure($dsDataValues, $dsName, 0, false, '', true);
				// 'true' says this outer structure is an array
			...
			// *** not a data structure. A regular element (could be an array or single
			// value). ***

			// data type. don't check for "empty" or $old[['type'], because 0 is legit
			// (CHAR).
			...
			list($whole, $dec) = explode('.', $length) + array(
				'',
				''
			);
			// extra array avoids Undefined index errors

			// TODO: apply same dim/array logic here that was used above for data structure
			// dim/arrays.
			//       But the need is not as urgent here because single-element arrays are not
			// as big (XML-wise)
			//       as data structures. Also, multiple occurrence data structures are more
			// prevalent
			...
			if ($useDefaultValues) {
				...
				if ($dim > 1) {
					...
					// TODO: should be able to use a single ProgramParameter object with the 'dim'
					// value set.
			...
			//$type,  $io, $comment='', $varName = '', $value, $varing = 'off', $dimension =
			// 0, $by = 'ref')
			$new = new ProgramParameter(sprintf($newType, $whole, $dec), // type
			// io
			$newInout,
			// comment
			$name,
			...
			$isArray);
			//$loggableValue = (is_array($dataValue)) ? print_r($dataValue, true) :
			// $dataValue;
			//            $connection->logThis("just said new ProgramParameter with name:
			// $name and dataValue: $loggableValue");
	...
	/**
	 * Take the previously given data description and match up with values and output
	 * params
	 * to make a parameter array/object that can be presented to a program or data
	 * queue, etc.
	 * If any error or validation problem occurs (such as a value not matching data
	 * type), return false.
	 *
	 *
	 * @param array $input                 Data input params
	 * @param array $description[optional] Description of data, if want to specify
	 * explicitly
	 *                                     rather than to use description from class
	 * member.
	...
	public function generateNewToolkitParams($input = array(), $description = null) {
		...
		foreach ($description as $key => $descParam) {
			...
			if ($needDefaultInput) {
				// only set this if didn't already turn on receiver-only globally. We don't want
				// to interfere then.
		...
		// TODO: when count/dim is specified, use it as max array size. otherwise error
		// 38 should occur.
	 ...
	 * in the constructor, and having converted the data desc and interpolated the
	 * values,
	 * Now we call the program and return an array of output variables.
	 *
	 * @param array $newInputParams   New-style toolkit array of params including
	 * values
	 *                                (use generateNewToolkitParams($inputValues) to
	 * create it)
	...
	public function callProgram($newInputParams = array()) {
		...
		$pgmCallOutput = $this->getConnection()->PgmCall($pgmName, $lib, $newInputParams,
		// null for returnvalue var because old toolkit doesn't handle return values (as
		// far as we can tell)
		null, $options);
	...
	protected function findInputValueByName($name, $inputArray) {
		foreach ($inputArray as $key => $value) {
			if ($key === $name) {// use === because plain == allowed numeric indexes to be
				// equal to names
				return $value;
			}
			if (is_array($value)) {
...
class DataDescriptionPcml extends DataDescription {
	/**
	 * Constructor takes a PCML string and converts to an array-based old toolkit
	...
	public function __construct($pcml, ToolkitService $connection) {
		...
		// Convert PCML from ANSI format (which old toolkit required) to UTF-8 (which
		// SimpleXML requires).

		//TODO this could possibly be done more efficiently, reading the ini-File again
		// is suboptimal
		...
		//program name is stored as: /pcml/program
		// name="/qsys.lib/eacdemo.lib/teststruc.pgm"
		...
		// let's focus on name, path, and entrypoint, the only attributes likely to be
		// used here.
		...
		// Note: if entrypoint is supplied, it's the function in a service program.
		// "name" will be the same as entrypoint.
		...
		//Change the encoding back to the one wanted by the user, since SimpleXML encodes
		// its output always in UTF-8
	...
	public function singlePcmlToArray(SimpleXmlElement $dataElement) {
		...
		if ($tagName == 'data') {
			...
			if ($type != 'struct') {
				...
				// regular type (char, int...), not a struct, so the data element's name is just
				// 'name'.
				$nameName = 'Name';
				...
				if (!$theStruct) {
					//				    $this->getConnection->logThis("PCML structure '$structName' not
					// found.");
					return null;
				}
				...
				// count can also be defined at the structure level. If so, it will override
				// count from data level)
				...
				// if we're not inheriting from our parent data element, but there is a struct
				// usage, use the struct's usage (input, output, or inputoutput).
				...
				if ($structSubDataElementsXmlObj) {
					foreach ($structSubDataElementsXmlObj as $subDataElementXmlObj) {
						...
						// here's where the recursion comes in. Convert data and add to array for our
						// struct.
			...
			//$struct = (isset($attrs['struct'])) ? (string) $attrs['struct'] : ''; // if
			// this is pointing to a struct name
	...
	// given an XML object containing a PCML program definition, return an old
	// toolkit style of data description array.
