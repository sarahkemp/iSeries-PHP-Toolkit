...
class XMLWrapper {
	...
	protected $errorText;
	protected $joblog;
	protected $joblogErrors = array();
	// cpf=>msgtext array

	protected $ToolkitSrvObj;

	// 'system' type can return CPF error codes.
	protected $_cmdTypes = array(
		'cmd',
		'rexx',
		'system'
	);

	// valid "on" values for varying. Can also be 'off' but that's the default so no
	// reason to specify it
	protected $_varyingTypes = array(
		'on',
		'2',
		'4'
	);
	...
	function __construct($options = '', ToolkitService $ToolkitSrvObj = null) {
		...
		if (is_string($options)) {
			// $options is a string so it must be encoding (assumption for backwards
			// compatibility)
	...
	// TODO: This works but is somewhat clumsy to use. Refactor as method of
	// ProgramParam object with XML creator injected into it.
	protected function processParamProps(&$props = array()) {

		$alphanumeric = true;
		// start with assumption
		// if type is not alphanumeric/char, get out of here! Hex has no purpose for
		// non-char.
	...
	protected function getPgmTag($pgm, $lib, $function) {
		...
		// if directed to use hex/CCSID, specify "long form" of program tag, with nested
		// name/lib/func tags.
		// If not using hex/CCSID, use "short form" of program tag.
		// Why allow both? For backward compatibility with older (pre-1.6.8) XMLSERVICE
		// that didn't support long form.
	...
	// TODO: $xml should be a property of the class (true OO style), then
	// $this->addOuterTags();
	...
	public function parseDiagnosticsXml($xml) {
		...
		if (isset($diag->jobinfo)) {
			...
			// ensure that all values are strings. Blanks might have remained Simple XML
			// Objects.
		 ...
		 *
		 ...
		 <jobtext><![CDATA[ASEIDEN QREXXMN qrexx_main 80 Object OUTREXX in QTEMP type
		*FILE not found.]]></jobtext>
		 </joblogrec>
	...
	public function buildXmlIn($inputOutputParams = NULL, array $returnParams = NULL, $pgm, $lib = "",
	// // blank library means use current/default or library list
	$function = NULL) {
	...
	protected function buildParamXml(ProgramParameter $paramElement) {
		 ...
		 * The inner DS or scalar field will be a template with a 'dim' attribute to be
		 * expanded on output from XMLSERVICE.
		 */
		/* if we add an outer DS that's "multi," don't bother to give the inner
		 * (original) ds a name.
		 * The inner ds will be repeated many times and its name will be replaced on
		 * output by numeric indexes.
		...
		if ($props['type'] == 'ds') {
			...
			// optional varying
			// varying only inserted if set on/2/4 (default is off, so we can let XMLSERVICE
			// supply the default behavior if off). The less XML we create and send, the more
			...
			// optional setLen to set length value to a numeric field (see 'len' for where
			...
			// use the old, inefficient "repeat item with sequential numbering of field name"
			// technique if backwards compatibility is desired
			...
			if ($useOldDimWay) {
				...
				// Use new, efficient, good way. Only one line needed with $dimStr, which
				// XMLSERVICE will expand for us.
				$innerNameStr = ($innerName) ? " var='$innerName'" : '';
				// only need name if exists. If not then it's probably a "multi" and doesn't need
				// an inner name.
		...
		// if a multi-occurrence DS or scalar field, wrap in an identially named "array"
		// DS shell.
		...
		if ($isMulti) {
			...
			// if not multi but regular old-style dim, an ordinary <ds> tag will do to
	...
	// given XMLSERVICE type such as 10i0, 5i0, 4f, 4p2, set data to the
	// corresponding PHP type as closely as possible.
	...
	// Parse a piece of XML representing a single parameter (whether output or return
	// type).
	...
	protected function getSingleParamFromXml(SimpleXMLElement $simpleXmlElement) {
		...
		$setTypes = $this->getIsCw();
		// do it if in CW mode because old toolkit did return correct types

		$element = array();

		// is this a parm, or perhaps a DS??
		$elementType = $simpleXmlElement->getName();
		// "name" of the XML tag is element type
		$elementAttrs = $simpleXmlElement->attributes();

		// if this is the outer (parm or return) element, go down one level to either ds
		// or data.
		...
		if ($elementType == 'ds') {
			...
			if ($underDs = $ds->xpath('data|ds')) {
				...
				foreach ($underDs as $indexNum => $subElement) {
					...
					if ($elType == 'ds') {
						...
						//echo "Array: " . printArray($data) . " with given name: $givenSubelementName
						// and nametouse: $subElementName<BR>";
						...
						if (isset($subElement['dtsdate']) && ($subElement['dtsdate'] == 'on') && $data && ($data != '4040404040404040')) {
			...
			// though if in "old" DS mode, don't include DS structure--just add elements
			// individually (single level).
			// check condition: if dataStructureIntegrity global setting or NEED that
			// integrity to interpret array/dim
			if ($this->getOption('dataStructureIntegrity') || $outerDsIsArray || $outerDsIsDim) {
				 ...
				 * Return without a name. Later that will be intepreted as being able to flatten
				 * the structure.
			...
			if (isset($attr['hex']) && $attr['hex'] == 'on') {
				$element[$dataVarName] = pack("H*", $element[$dataVarName]);
				// reverse of bin2hex()
			}
			...
			if ($setTypes) {
				$type = $attr->type;
				$this->updateType($data, $type);
				// TODO interesting--shouldn't this be $element[$dataVarName], not $data?
			}
			// Similar to code above in "single data element under a DS"
			// if data is a DTS date that needs to be converted to a regular date.
			// and data is not empty or blank (hex ebcdic)
			// TODO could convert 4040 into blanks
			if (isset($attr->dtsdate) && (((string)$attr->dtsdate) == 'on') && $element[$dataVarName] && ($element[$dataVarName] != '4040404040404040')) {
				if (!isset($dateTimeApi)) {
					$dateTimeApi = new DateTimeApi($this->ToolkitSrvObj);
				}
				// replace DTS date with "real" date
	...
	// parse it into an array of parameter key/value pairs (can be nested if complex
	// DSes) ready to use.
	// The array will contain both 'io_param' (regular output params) and 'retvals'
	// (return param) elements.
	public function getParamsFromXml($xml) {

		// initialize results array to empty arrays for both regular in/out params and
		// return params
		$callResults = array(
			'io_param' => array(),
			'retvals' => array()
		...
		if (isset($xmlobj->error)) {
			...
			$this->error = (string)$xmlobj->error->xmlerrmsg . ' (' . (string)$xmlobj->error->xmlhint . ')';
		...
		foreach ($xmlParamElements as $paramType => $elements) {
			...
			foreach ($elements as $simpleXmlElement) {
				...
				if ($paramName) {
					...
					// structure name is blank. Append array elements as if they were separate
					// values.
	...
	// It's more efficient to run multiple commands with one XMLSERVICE call than to
	// do many calls
	public function buildCommandXmlIn($cmd, $exec = 'pase') {
		...
		foreach ($cmdArray as $oneCmd) {
			...
			// We call this an "interactive" command because it can return output from DSP*
			...
			if ($exec == 'pase' || $exec == 'pasecmd') {
				...
				if ($exec && in_array($exec, $this->_cmdTypes)) {
					...
					$execStr = '';
					// OK because blank string defaults to cmd on XMLSERVICE side
				}
	...
	public function getCmdResultFromXml($xml, $parentTag = 'sh') {
		...
		$retval = true;
		// assume OK unless error found. Was false, but sh doesn't return success flag.
		...
		foreach ($params as $simpleXMLElement) {
			...
			if (isset($simpleXMLElement->error)) {//*** error
				...
				// get last joblogscan/joblogrec nested tags in the XML section. Returned as
				// one-element array. function 'current' returns the one element.
				$lastJoblogTag = current($simpleXMLElement->xpath("joblogscan/joblogrec[last()]"));

				// if last job log tag contained a real CPF-style code, of length 7 (e.g.
				// CPF1234), parsed for us.
				if ($lastJoblogTag && isset($lastJoblogTag->jobcpf) && (strlen($lastJoblogTag->jobcpf) == 7)) {
					...
					// ->error has ambiguous meaning. Include for backward compatibility.
				 ...
				 * <joblogscan>
				 ...
				 <jobtext><![CDATA[Job 127257/ASEIDEN/XTOOLKIT started on 10/11/12 at Job
				127257/ASEIDEN/XTOOLKIT submitted.]]></jobtext>
				 </joblogrec>
				 <joblogrec>
				 <jobcpf>*NONE</jobcpf>
				 <jobtime><![CDATA[10/11/12  19:33:54.102423]]></jobtime>
				 <jobtext><![CDATA[CALL PGM(XMLSERVICE/XMLSERVICE)
				PARM('/tmp/ASEIDEN_abc')]]></jobtext>
				 </joblogrec>
				 <joblogrec>
				 <jobcpf>CPF3142</jobcpf>
				 <jobtime><![CDATA[10/11/12  19:33:54.102423]]></jobtime>
				 <jobtext><![CDATA[PLUGILE ILECMDEXC 5116 File NONEXIST in library *LIBL not
				found.]]></jobtext>
				 </joblogrec>
				 </joblogscan>
				 */
	...
	public function getRowsFromXml($xml, $parent = 'sh') {
		...
		foreach ($params as $simpleXMLElement) {
			...
			// Then can un-hex (pack) that and do simplexml load string on it, and proceed as
			// below afterward.
			// Examples from XMLSERVICE documentation:
			 ...
			 *       <row><data
			 * desc='USRLIBL'><hex><![CDATA[5147504C20202020202020]]></hex></data></row>
			 *       <row><data
			 * desc='SYSLIBL'><hex><![CDATA[5153595320202020202020]]></hex></data></row>
			 *   </cmd>
			 */
			...
			// if a hex tag is present, de-hex (pack) the data inside that hex tag;
			// otherwise, use the outer element's value directly.
			...
			if ($parent == 'cmd') {
				// for RTV* commands,
				// attribute 'desc' of data tag will hold the output field name to which the data
				// belongs.
				$outputFieldName = (string)$element->attributes()->desc;

				$data = trim($data, " \n");
				// trim spaces and end-of-line chars of data that comes from REXX RTV* commands
				// ('cmd')
